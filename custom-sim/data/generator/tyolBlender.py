...
from data.renderer.tyolBlender import TYOLBlenderRenderer
from data.features.tyol import TYOLFeatures

class TYOLBlenderGenerator(Generator):
  """
  ...
  def __init__(self, config):
    super(TYOLBlenderGenerator, self).__init__(config, 
      renderer=True,
      features=True
    )
  ...
  def _init_renderer(self):
    self.renderer = TYOLBlenderRenderer(self.config)
  ...
  def _sample_attributes(self, attr, node_class):
    ...
    """
    out = super(TYOLBlenderGenerator, self)._sample_attributes(attr, node_class)
