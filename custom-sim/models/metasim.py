...
class MetaSim(nn.Module):
  def __init__(self, opts, dropout=None):
  ...
  def init_model(self, dropout=None):
  ...
  def sample(self, features, masks, sigma=None):
    ...
    """
    if sigma is None:
      sigma = torch.tensor(0.02).to(self.opts['device']) # Initially 0.02
    ...
    return features, log_probs # Log probability for node prediction