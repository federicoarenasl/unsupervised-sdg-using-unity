...
from models.layers.mmd_geomloss import MMDGeomInception
import time
import shutil
...
class Trainer(object):
  def __init__(self, opts):
    ...
    if opts['geomloss']['use_geomloss']:
      self.mmd = MMDGeomInception(device=self.device, resize_input=self.opts['mmd_resize_input'], 
                                  out_features=self.opts['batch_size'], loss_type=self.opts['geomloss']['loss_metric'])
    else:
  ...
  def train_MMD(self):
    ...
    if self.opts['dataset'] == 'mnist':
      mmd_train_prog = {'epoch':[], 'mmd_loss':[], 'task_loss':[], 'loss':[], 'task_acc':[], 'dataset_gen_time':[], 'epoch_runtime':[]}
    else:
    ...
    for e in tqdm(range(self.last_epoch, self.opts['max_epochs']+1), desc="Training Meta-Sim"):
      ...
      with torch.no_grad():
        ...
        for idx, (g, x, m, adj) in tqdm(enumerate(loader), desc='Generating Data'):
          ...
          for k in range(len(g)): 
            ...
            img, lbl = r[k]
            io.output_recent_data(im_dir, graph_dir, img, lbl, f[k], i_g, 'jpg')
            # Save key epoch distribution of images and graphs
            if e%self.opts['distprint_freq'] == 0:
              io.output_epoch_data(self.imagedir, self.graphdir, e, img, lbl, f[k], i_g,'jpg')
            i_g+=1
      ...
      acc = self.tasknet.train_from_dir(im_dir)
      ...
      if self.opts['dataset'] == 'mnist':
        io.save_mmd_progress_mnist(e, mmd, mmd_train_prog,\
                                    task_loss, loss, acc,\
                                    egen_time, etime, self.csvdir)
      else:
